/**
  ******************************************************************************
  * @file    crc16.c
  * @author  Miroslaw Lach
  * @version V.2.0
  * @date    18.08.2011
  * @brief   Procedura obliczeniowa CRC-16
  ******************************************************************************
  * @copyright
  *
  * <h2><center>&copy; COPYRIGHT 2011 Miroslaw Lach</center></h2>
  */ 

/* Includes ------------------------------------------------------------------*/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
//#include "stm32f10x.h"
#include "crc16.h"
// Table of CRC values for high-order byte

const unsigned char auchCRCHi[] ={
 } ;

// Table of CRC values for low-order byte
const unsigned char auchCRCLo[] ={
 };



unsigned short CalcCrc16Fast(unsigned char *Buffer,unsigned short Length,unsigned short CRCInit){
unsigned char uchCRCHi = CRCInit>>8;                         // high CRC byte initialized
unsigned char uchCRCLo = CRCInit;                         // low CRC byte initialized
unsigned char uIndex ;                                   // will index into CRC lookup

  while (Length--)                                     // pass through message buffer
  {
    uIndex=uchCRCHi ^ *Buffer++ ;           // calculate the CRC
    uchCRCHi=uchCRCLo ^ (auchCRCHi[uIndex]);
    uchCRCLo=(auchCRCLo[uIndex]) ;
//    WDGReload();
  }
  return (uchCRCHi*256+uchCRCLo);
}/**/

unsigned short CalcCrc16Fast_b(unsigned char *Buffer,unsigned short Length){
  return CalcCrc16Fast(Buffer,Length,0xffff);
}

